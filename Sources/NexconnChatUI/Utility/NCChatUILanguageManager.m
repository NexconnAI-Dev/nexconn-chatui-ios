...
/// When no language is explicitly configured, this stores the resolved system language rather than
/// an empty string.
...
- (instancetype)init {
    if (self = [super init]) {
        _syncQueue =
            dispatch_queue_create("ai.nexconn.imkit.languageManager", DISPATCH_QUEUE_SERIAL);
        ...
        // Initialization reads the current system preference when no explicit language is
        // configured. Call reloadLanguageContext after a relevant preference change to rebuild
        // candidates and clear cached bundles.
    }
    return self;
}
...
- (void)reloadLanguageContext {
    dispatch_async(self.syncQueue, ^{
      ...
      // Compare the resolved language, including the system preference when no explicit language is
      // set.
...
- (NSArray<NSString *> *)p_fallbackChainForLanguage:(NSString *)langId {
    ...
    if (components.count > 1) {
        for (NSInteger i = components.count - 1; i > 0; i--) {
            NSString *parent =
                [[components subarrayWithRange:NSMakeRange(0, i)] componentsJoinedByString:@"-"];
...
- (NSBundle *)p_findBundleForTable:(NSString *)table {
    NSArray *bundles =
        @[ [NSBundle mainBundle], [NSBundle bundleForClass:[NCChatUIUtility class]] ];
