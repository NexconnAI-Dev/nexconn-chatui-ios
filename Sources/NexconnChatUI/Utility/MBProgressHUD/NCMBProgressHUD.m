...
// NCMBProgressHUD.m
// Version 1.0.0
// Adapted from MBProgressHUD: https://github.com/jdg/MBProgressHUD
// Original copyright (c) 2009-2016 Matej Bukovinski.
...
#define MBMainThreadAssert()                                                                       \
    NSAssert([NSThread isMainThread], @"NCMBProgressHUD needs to be accessed on the main "         \
                                      @"thread.");

CGFloat const NCMBProgressMaxOffset = 1000000.f;
...
@interface NCMBProgressHUDRoundedButton : UIButton
@end

@implementation NCMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    NCMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    NCMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (NCMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (NCMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = NCMBProgressHUDAnimationFade;
    _mode = NCMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn
         withType:(NCMBProgressHUDAnimation)type
       completion:(void (^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == NCMBProgressHUDAnimationZoom) {
        type = animatingIn ? NCMBProgressHUDAnimationZoomIn : NCMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == NCMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == NCMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
      ...
      } else if (!animatingIn && type == NCMBProgressHUDAnimationZoomIn) {
          bezelView.transform = large;
      } else if (!animatingIn && type == NCMBProgressHUDAnimationZoomOut) {
          bezelView.transform = small;
      }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    NCMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<NCMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    NCMBBackgroundView *backgroundView = [[NCMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = NCMBProgressHUDBackgroundStyleSolidColor;
    ...
    NCMBBackgroundView *bezelView = [NCMBBackgroundView new];
    ...
    UIButton *button = [NCMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    ...
    for (UIView *view in @[ label, detailsLabel, button ]) {
        ...
        [view setContentCompressionResistancePriority:998.f
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[NCMBRoundProgressView class]];

    NCMBProgressHUDMode mode = self.mode;
    if (mode == NCMBProgressHUDModeIndeterminate) {
    ...
    } else if (mode == NCMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[NCMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    } else if (mode == NCMBProgressHUDModeDeterminate ||
               mode == NCMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[NCMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == NCMBProgressHUDModeAnnularDeterminate) {
            [(NCMBRoundProgressView *)indicator setAnnular:YES];
        }
    } else if (mode == NCMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    } else if (mode == NCMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
    ...
    [indicator setContentCompressionResistancePriority:998.f
                                               forAxis:UILayoutConstraintAxisHorizontal];
    [indicator setContentCompressionResistancePriority:998.f
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance =
            [UIActivityIndicatorView appearanceWhenContainedIn:[NCMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView
            appearanceWhenContainedInInstancesOfClasses:@[ [NCMBProgressHUD class] ]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[NCMBRoundProgressView class]]) {
        NCMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [NCMBRoundProgressView appearanceWhenContainedIn:[NCMBProgressHUD class], nil];
#else
        appearance = [NCMBRoundProgressView
            appearanceWhenContainedInInstancesOfClasses:@[ [NCMBProgressHUD class] ]];
#endif
        if (appearance.progressTintColor == nil) {
            ((NCMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((NCMBRoundProgressView *)indicator).backgroundTintColor =
                [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[NCMBBarProgressView class]]) {
        NCMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [NCMBBarProgressView appearanceWhenContainedIn:[NCMBProgressHUD class], nil];
#else
        appearance = [NCMBBarProgressView
            appearanceWhenContainedInInstancesOfClasses:@[ [NCMBProgressHUD class] ]];
#endif
        if (appearance.progressColor == nil) {
            ((NCMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((NCMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    NCMBBackgroundView *bezelView = self.bezelView;
...
- (void)updateConstraints {
    ...
    NSMutableArray *subviews =
        [NSMutableArray arrayWithObjects:self.topSpacer, self.label, self.detailsLabel, self.button,
    ...
    [sideConstraints
        addObjectsFromArray:[NSLayoutConstraint
                                constraintsWithVisualFormat:@"|-(>=margin)-[bezel]-(>=margin)-|"
                                                    options:0
                                                    metrics:metrics
                                                      views:NSDictionaryOfVariableBindings(bezel)]];
    [sideConstraints
        addObjectsFromArray:[NSLayoutConstraint
                                constraintsWithVisualFormat:@"V:|-(>=margin)-[bezel]-(>=margin)-|"
                                                    options:0
                                                    metrics:metrics
                                                      views:NSDictionaryOfVariableBindings(bezel)]];
    ...
    if (!CGSizeEqualToSize(minimumSize, CGSizeZero)) {
        ...
        [minSizeConstraints
            addObject:[NSLayoutConstraint constraintWithItem:bezel
    ...
    [topSpacer
        addConstraint:[NSLayoutConstraint constraintWithItem:topSpacer
                                                   attribute:NSLayoutAttributeHeight
                                                   relatedBy:NSLayoutRelationGreaterThanOrEqual
                                                      toItem:nil
                                                   attribute:NSLayoutAttributeNotAnAttribute
                                                  multiplier:1.f
                                                    constant:margin]];
    [bottomSpacer
        addConstraint:[NSLayoutConstraint constraintWithItem:bottomSpacer
                                                   attribute:NSLayoutAttributeHeight
                                                   relatedBy:NSLayoutRelationGreaterThanOrEqual
                                                      toItem:nil
                                                   attribute:NSLayoutAttributeNotAnAttribute
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
      ...
      [bezelConstraints
          addObjectsFromArray:[NSLayoutConstraint
                                  constraintsWithVisualFormat:@"|-(>=margin)-[view]-(>=margin)-|"
                                                      options:0
                                                      metrics:metrics
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints
        enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
          UIView *firstView = (UIView *)padding.firstItem;
          UIView *secondView = (UIView *)padding.secondItem;
          BOOL firstVisible =
              !firstView.hidden && !CGSizeEqualToSize(firstView.intrinsicContentSize, CGSizeZero);
          BOOL secondVisible =
              !secondView.hidden && !CGSizeEqualToSize(secondView.intrinsicContentSize, CGSizeZero);
          // Set if both views are visible or if there's a visible view on top that doesn't have
          // padding added relative to the current view yet
          padding.constant =
              (firstVisible && (secondVisible || hasVisibleAncestors)) ? MBDefaultPadding : 0.f;
...
- (void)setMode:(NCMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == NCMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)setProgressObjectDisplayLink:(CADisplayLink *)progressObjectDisplayLink {
    if (progressObjectDisplayLink != _progressObjectDisplayLink) {
        ...
        [_progressObjectDisplayLink addToRunLoop:[NSRunLoop mainRunLoop]
...
- (void)setNSProgressDisplayLinkEnabled:(BOOL)enabled {
    // We're using CADisplayLink, because NSProgress can change very quickly and observing it may
    // starve the main thread, so we're refreshing the progress only every frame draw
...
// to avoid sharedApplication problems on extension targets
    ...
    if (UIInterfaceOrientationIsLandscape(orientation)) {
        radians =
            orientation == UIInterfaceOrientationLandscapeLeft ? -(CGFloat)M_PI_2 : (CGFloat)M_PI_2;
...
@implementation NCMBRoundProgressView
...
- (void)setProgressTintColor:(UIColor *)progressTintColor {
    ...
    if (progressTintColor != _progressTintColor &&
        ![progressTintColor isEqual:_progressTintColor]) {
...
- (void)setBackgroundTintColor:(UIColor *)backgroundTintColor {
    ...
    if (backgroundTintColor != _backgroundTintColor &&
        ![backgroundTintColor isEqual:_backgroundTintColor]) {
...
@implementation NCMBBarProgressView
...
- (void)drawRect:(CGRect)rect {
    ...
    if (amount >= radius + 4 && amount <= (rect.size.width - radius - 4)) {
        ...
        CGContextMoveToPoint(context, 4, rect.size.height / 2);
        CGContextAddArcToPoint(context, 4, rect.size.height - 4, radius + 4, rect.size.height - 4,
                               radius);
    ...
    // Progress in the right arc
        ...
        if (isnan(angle))
            angle = 0;
        CGContextAddArc(context, rect.size.width - radius - 4, rect.size.height / 2, radius, M_PI,
                        angle, 0);
...
@implementation NCMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = NCMBProgressHUDBackgroundStyleBlur;
            ...
            _style = NCMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(NCMBProgressHUDBackgroundStyle)style {
    if (style == NCMBProgressHUDBackgroundStyleBlur &&
        kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = NCMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    NCMBProgressHUDBackgroundStyle style = self.style;
    if (style == NCMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
#if !TARGET_OS_TV
            UIToolbar *toolbar =
                [[UIToolbar alloc] initWithFrame:CGRectInset(self.bounds, -100.f, -100.f)];
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == NCMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation NCMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [NCMBProgressHUD allHUDsForView:view];
    for (NCMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (BOOL)dimBackground {
    NCMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor = [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == NCMBProgressHUDBackgroundStyleSolidColor &&
           [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = NCMBProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color =
        dimBackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation NCMBProgressHUDRoundedButton
...
- (void)setHighlighted:(BOOL)highlighted {
    ...
    self.backgroundColor =
        highlighted ? [baseColor colorWithAlphaComponent:0.1f] : [UIColor clearColor];
}
