...
#import "NCFMDatabaseAdditions.h"
...
- (NSString *)stringForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(NSString *, stringForColumnIndex)
}

- (int)intForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(int, intForColumnIndex)
}

- (long)longForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(long, longForColumnIndex)
}

- (BOOL)boolForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(BOOL, boolForColumnIndex)
}

- (double)doubleForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(double, doubleForColumnIndex)
}

- (NSData *)dataForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(NSData *, dataForColumnIndex)
}

- (NSDate *)dateForQuery:(NSString *)query, ... {
    NCFMDB_RETURN_RESULT_FOR_QUERY_WITH_SELECTOR(NSDate *, dateForColumnIndex)
}
...
- (BOOL)columnExists:(NSString *)columnName inTableWithName:(NSString *)tableName {
    ...
    NCFMResultSet *rs = [self getTableSchema:tableName];
    ...
    // If this is not done NCFMDatabase instance stays out of pool
    [rs close];
...
- (uint32_t)applicationID {
    ...
    NCFMResultSet *rs = [self executeQuery:@"pragma application_id"];
...
- (void)setApplicationID:(uint32_t)appID {
    ...
    NCFMResultSet *rs = [self executeQuery:query];
    [rs next];
    [rs close];
#else
    NSString *errorMessage = NSLocalizedString(@"Application ID functions require SQLite 3.7.17", nil);
...
- (uint32_t)userVersion {
    ...
    NCFMResultSet *rs = [self executeQuery:@"pragma user_version"];
