...
static int NCFMDBDatabaseBusyHandler(void *f, int count) {
    NCFMDatabase *self = (__bridge NCFMDatabase *)f;
...
#pragma mark NCFMDatabase instantiation and deallocation
...
- (void)dealloc {
    [self close];
    NCFMDBRelease(_openResultSets);
    NCFMDBRelease(_cachedStatements);
    NCFMDBRelease(_dateFormat);
    NCFMDBRelease(_databasePath);
    NCFMDBRelease(_openFunctions);
...
// if ([NCFMDatabase NCFMDBVersion] >= 0x0240) { … }
...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
    ...
    return YES;
#else
    NCLogD(@"openWithFlags requires SQLite 3.5");
    return NO;
#endif
}
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) != 0) {
                    NCLogD(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                    retry = YES;
                }
            }
        } else if (SQLITE_OK != rc) {
            NCLogD(@"error closing!: %d", rc);
        }
    } while (retry);
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &NCFMDBDatabaseBusyHandler, (__bridge void *)(self));
    } else {
...
// but for folks who don't bother noticing that the interface to NCFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NCLogD(@"%s:%d", __FUNCTION__, __LINE__);
    NCLogD(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NCLogD(@"%s:%d", __FUNCTION__, __LINE__);
    NCLogD(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)resultSetDidClose:(NCFMResultSet *)resultSet {
...
- (NCFMStatement *)cachedStatementForQuery:(NSString *)query {
...
- (void)setCachedStatement:(NCFMStatement *)statement forQuery:(NSString *)query {
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        NCLogD(@"error on rekey: %d", rc);
        NCLogD(@"%@", [self lastErrorMessage]);
    }
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = NCFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = NCFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    NCFMDBAutorelease(_dateFormat);
    _dateFormat = NCFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    NCFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NCLogD(@"The NCFMDatabase %@ is currently in use.", self);

#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The NCFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NCLogD(@"The NCFMDatabase %@ is not open.", self);

#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The NCFMDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NCFMResultSet *)executeQuery:(NSString *)sql
    ...
    NCFMStatement *statement = 0x00;
    NCFMResultSet *rs = 0x00;

    if (_traceExecution && sql) {
        NCLogD(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                NCLogD(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                NCLogD(@"DB Query: %@", sql);
                NCLogD(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                NCLogD(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }

            // Get the index for the parameter name.
            int namedIdx = sqlite3_bind_parameter_index(pStmt, [parameterName UTF8String]);

            NCFMDBRelease(parameterName);
            ...
            if (namedIdx > 0) {
                ...
                NCLogD(@"Could not find index for %@", dictionaryKey);
            }
        }
    } else {
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    NCLogD(@"data: %ld bytes", (unsigned long)[(NSData *)obj length]);
                } else {
                    NCLogD(@"obj: %@", obj);
                }
            }
    ...
    if (idx != queryCount) {
        NCLogD(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }

    NCFMDBRetain(statement); // to balance the release below

    if (!statement) {
        statement = [[NCFMStatement alloc] init];
    ...
    rs = [NCFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (NCFMResultSet *)executeQueryWithFormat:(NSString *)format, ... {
    va_list args;
...
- (BOOL)executeUpdate:(NSString *)sql
 ...
 withArgumentsInArray:(NSArray *)arrayArgs
    ...
    int rc = 0x00;
    sqlite3_stmt *pStmt = 0x00;
    NCFMStatement *cachedStmt = 0x00;

    if (_traceExecution && sql) {
        NCLogD(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (SQLITE_ERROR == rc) {
            ...
            // wtf?
            if (_logsErrors) {
                NCLogD(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
                NCLogD(@"DB Query: %@", sql);
            }
        }
    }
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[NCFMStatement alloc] init];

        [cachedStmt setStatement:pStmt];

        [self setCachedStatement:cachedStmt forQuery:sql];

        NCFMDBRelease(cachedStmt);
    }
...
- (void)makeFunctionNamed:(NSString *)name
    ...
    id b = NCFMDBReturnAutoreleased([block copy]);
