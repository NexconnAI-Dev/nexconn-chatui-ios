...
+ (instancetype)resultSetWithStatement:(NCFMStatement *)statement
    ...
    return NCFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    NCFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return NCFMDBReturnAutoreleased([dict copy]);
    } else {
        NCLogD(@"Warning: There seem to be no columns in this set.");
    }
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        NCLogD(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        NCLogD(@"Database busy");
        if (outErr) {
...
- (int)columnIndexForName:(NSString *)columnName {
    ...
    NCLogD(@"Warning: I could not find the column named '%@'.", columnName);
...
- (void)setParentDB:(NCFMDatabase *)newDb {
    _parentDB = newDb;
}
