...
+ (void)checkNotifyAbilityWith:(NCMessage *)message completion:(void (^)(BOOL show))completion {
    ...
    [NCChannelNotificationDataContext
        queryNotificationLevelWithChannelIdentifier:message.channelIdentifier
...
+ (BOOL)shouldPushByLevel:(NCChannelNoDisturbLevel)level message:(NCMessage *)message {
    ...
    case NCChannelNoDisturbLevelDefaultLevel:
        break;
        // Direct and system channels do not notify. Other channel types notify for any mention of
        // the current user.
    case NCChannelNoDisturbLevelMention: {
        ...
        // Direct and system channels do not notify. Other channel types require a non-@all mention
        // of the current user.
    ...
    NCUILog(@"channelType: %ld -> show: %@ -> level: %ld",
            (long)message.channelIdentifier.channelType, ret ? @"YES" : @"NO", (long)level);
