...
static NSString *NCProfileCurrentUserId(void) {
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.channelEventHandlerId = [NSString stringWithFormat:@"nc.user.profile.channel.%p", self];
...
- (void)viewController:(UIViewController *)viewController tableView:(UITableView *)tableView didSelectRow:(NSIndexPath *)indexPath {
    ...
    if ([commonCellViewModel.title isEqualToString:NCUILocalizedString(@"set_remark")]) {
        NCNameEditViewModel *viewModel = [NCNameEditViewModel viewModelWithUserId:self.userId groupId:nil type:NCNameEditTypeRemark];
    ...
    } else if ([commonCellViewModel.title isEqualToString:NCUILocalizedString(@"group_member_nickname")] && [self canEditGroupMemberNickname]) {
        NCNameEditViewModel *viewModel = [NCNameEditViewModel viewModelWithUserId:self.userId groupId:self.group.groupId type:NCNameEditTypeGroupMemberNickname];
...
- (void)updateProfile {
    ...
    if (self.verifyFriend) {
        [[NCEngine userModule] checkFriendsWithUserIds:@[self.userId]
                                            completion:^(NSArray<NCFriendRelationInfo *> * _Nullable friendRelations, NCError * _Nullable error) {
            if (error) {
                [self getUserProfile];
                return;
            }
            NCFriendRelationInfo *relationInfo = friendRelations.firstObject;
            if (relationInfo.relationType == NCFriendRelationTypeInMyFriendList || relationInfo.relationType == NCFriendRelationTypeBothWay) {
                self.isFriend = YES;
                [self getFriendInfo];
            } else {
                [self getUserProfile];
...
- (void)getUserProfile {
    [[NCEngine userModule] getUserProfilesWithUserIds:@[self.userId ? : @""]
...
- (void)loadFooterViewModel {
    ...
    NCProfileFooterViewModel *footerViewModel = [[NCProfileFooterViewModel alloc] initWithResponder:[self.responder currentViewController] type:type channelId:self.userId];
...
- (NSArray<NSArray<NCProfileCellViewModel *> *> *)reloadFriendDataSource:(NCFriendInfo *)friendInfo {
    
    NSMutableArray *profileList = [NSMutableArray array];
    
    NCUserProfileHeaderCellViewModel *headerVM = [[NCUserProfileHeaderCellViewModel alloc] initWithPortrait:friendInfo.avatarUrl name:friendInfo.name remark:friendInfo.remark];
    // Online status.
    [self setupViewModelOnlineStatus:headerVM];
    
    NCProfileCommonCellViewModel *setRemarkVM = [[NCProfileCommonCellViewModel alloc] initWithCellType:NCUProfileCellTypeText title:NCUILocalizedString(@"set_remark") detail:nil];
    ...
    if (self.member) {
        NCProfileCommonCellViewModel *memberNicknameVM = [[NCProfileCommonCellViewModel alloc] initWithCellType:NCUProfileCellTypeText title:NCUILocalizedString(@"group_member_nickname") detail:self.member.nickname];
...
- (NSArray<NSArray<NCProfileCellViewModel *> *> *)reloadDataSource:(NCUserProfile *)userProfile {
    ...
    NCUserProfileHeaderCellViewModel *headerVM = [[NCUserProfileHeaderCellViewModel alloc] initWithPortrait:userProfile.avatarUrl name:userProfile.name remark:userProfile.email];
...
- (BOOL)canEditGroupMemberNickname {
    ...
    if (self.group.memberInfoEditPermission == NCGroupMemberInfoEditPermissionOwnerOrAdminOrSelf && (self.group.role == NCGroupMemberRoleOwner || self.group.role == NCGroupMemberRoleAdmin)) {
        return YES;
    }
...
- (NCSubscribeUserOnlineStatus *)getUserOnlineStatus:(NSString *)userId {
    ...
    if (!onlineStatus) {
        if (self.isFriend && self.verifyFriend) {
            ...
            [NCUserOnlineStatusManager.sharedManager fetchOnlineStatus:userId processSubscribeLimit:NO];
        }
    }
    return onlineStatus;
}
