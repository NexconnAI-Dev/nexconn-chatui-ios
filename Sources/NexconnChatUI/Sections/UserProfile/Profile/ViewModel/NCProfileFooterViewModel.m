...
- (instancetype)initWithResponder:(UIViewController *)responder type:(NCProfileFooterViewType)type channelId:(NSString *)channelId {
    self = [super init];
    if (self) {
...
- (UIView *)loadView {
    NCProfileFooterView *footerView = [[NCProfileFooterView alloc] initWithTopSpace:[self topSpace] buttonSpace:[self buttonSpace] items:[self items]];
    return footerView;
}
...
- (NSArray *)userItems {
    NCButtonItem *chatItem = [NCButtonItem itemWithTitle:NCUILocalizedString(@"start_chat")
    ...
    [chatItem setClickBlock:^{
        NCChannelViewController *conversationVC = [[NCChannelViewController alloc] initWithChannelType:NCChannelTypeDirect channelId:weakSelf.channelId];
        [weakSelf.responder.navigationController pushViewController:conversationVC animated:YES];
    }];
    
    if (self.verifyFriend) {
        NCButtonItem *deleteItem = [NCButtonItem itemWithTitle:NCUILocalizedString(@"delete_friend")
                                                    titleColor:NCDynamicColor(@"hint_color")
                                               backgroundColor:NCDynamicColor(@"common_background_color")];
        deleteItem.borderColor =
        NCDynamicColor(@"clear_color");
        [deleteItem setClickBlock:^{
            [[NCEngine userModule] getUserProfilesWithUserIds:@[weakSelf.channelId]
                                                   completion:^(NSArray<NCUserProfile *> * _Nullable userProfiles, NCError * _Nullable error) {
                if (error) {
                    return;
                }
                dispatch_async(dispatch_get_main_queue(), ^{
                    [NCAlertView showAlertController:NCUILocalizedString(@"tip") message:[NSString stringWithFormat:NCUILocalizedString(@"delete_friend_alert"), userProfiles.firstObject.name] actionTitles:nil cancelTitle:NCUILocalizedString(@"cancel") confirmTitle:NCUILocalizedString(@"confirm") preferredStyle:UIAlertControllerStyleAlert actionsBlock:nil cancelBlock:^{
                    } confirmBlock:^{
...
- (void)deleteFriend {
    [[NCEngine userModule] removeFriendsWithUserIds:@[self.channelId ?: @""]
                                         completion:^(NCError * _Nullable error) {
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [NCAlertView showAlertController:nil message:NCUILocalizedString(@"delete_friend_failed") hiddenAfterDelay:2];
            });
            return;
        }
        NCDirectChannel *channel = [[NCDirectChannel alloc] initWithChannelId:self.channelId ?: @""];
        ...
        NCDeleteMessagesForMeByTimestampParams *params = [[NCDeleteMessagesForMeByTimestampParams alloc] initWithTimestamp:0
                                                                                                                     policy:NCMessageOperationPolicyLocalRemote];
        [channel deleteMessagesForMeByTimestampWithParams:params completion:^(NCError * _Nullable error) {
            if (error) {
                NCLogE(@"Contact deleted, but clearing messages failed");
            }
        }];
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [NCAlertView showAlertController:nil message:NCUILocalizedString(@"delete_friend_success") hiddenAfterDelay:1];
        });
    }];
}
...
- (NSArray *)groupMemberItems {
    NCButtonItem *item = [NCButtonItem itemWithTitle:NCUILocalizedString(@"group_action_quit") titleColor:NCDynamicColor(@"control_title_white_color") backgroundColor:NCDynamicColor(@"hint_color")];
    item.borderColor =
    NCDynamicColor(@"clear_color");
    __weak typeof(self) weakSelf = self;
    [item setClickBlock:^{
...
- (void)dismissGroup {
    ...
    [channel dismissWithCompletion:^(NCError * _Nullable error) {
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [NCAlertView showAlertController:nil message:NCUILocalizedString(@"group_dismiss_failed") hiddenAfterDelay:2];
            });
            return;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf.responder.navigationController popViewControllerAnimated:YES];
            [NCAlertView showAlertController:nil message:NCUILocalizedString(@"group_dismiss_success") hiddenAfterDelay:1];
        });
    }];
}
...
- (void)quitGroup {
    ...
    [channel leaveWithConfig:nil completion:^(NCError * _Nullable error) {
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [NCAlertView showAlertController:nil message:NCUILocalizedString(@"group_quit_failed") hiddenAfterDelay:2];
            });
            return;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf.responder.navigationController popViewControllerAnimated:YES];
            [NCAlertView showAlertController:nil message:NCUILocalizedString(@"group_quit_success") hiddenAfterDelay:2];
        });
    }];
}
...
- (void)addFriend:(NSString *)text{
    ...
    params.extra = text;
    [[NCEngine userModule] addFriendWithParams:params completion:^(NSInteger processCode, NCError * _Nullable error) {
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [NCAlertView showAlertController:nil message:NCUILocalizedString(@"add_friend_failed") hiddenAfterDelay:2];
            });
            return;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [NCAlertView showAlertController:nil message:NCUILocalizedString(@"friend_request_has_sent") hiddenAfterDelay:2];
        });
    }];
}
@end
