...
- (void)setupConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        [self.membersView.leadingAnchor
            constraintEqualToAnchor:self.paddingContainerView.leadingAnchor],
        [self.membersView.trailingAnchor
            constraintEqualToAnchor:self.paddingContainerView.trailingAnchor],
        [self.membersView.topAnchor constraintEqualToAnchor:self.paddingContainerView.topAnchor
                                                   constant:NCGroupProfileMembersCellTextTopSpace],
        [self.membersView.bottomAnchor
...
- (NCGroupMembersCollectionView *)membersView {
    if (!_membersView) {
        ...
        layout.itemSize =
            CGSizeMake(NCGroupMembersCollectionViewModelItemWidth,
                       NCGroupMembersCollectionViewModelItemHeight); // Size of each item.
        layout.minimumLineSpacing = NCGroupMembersCollectionViewModelLineSpace;
        layout.sectionInset = UIEdgeInsetsMake(0, NCGroupMembersCollectionViewModelLineSpace, 0,
                                               NCGroupMembersCollectionViewModelLineSpace);
        CGFloat width = [UIScreen mainScreen].bounds.size.width;
        CGFloat padding = (width - 32 - 5 * layout.itemSize.width -
                           NCGroupMembersCollectionViewModelLineSpace * 2) /
                          4;
        ...
        _membersView = [[NCGroupMembersCollectionView alloc] initWithFrame:CGRectZero
