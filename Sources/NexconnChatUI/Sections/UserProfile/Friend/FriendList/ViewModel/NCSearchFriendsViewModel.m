...
- (instancetype)init {
    ...
    if (self) {
        self.queue =
            dispatch_queue_create("ai.nexconn.searchFriends.operationQueue", DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(self.queue, NCSearchFriendsOperationQueueSpecificKey,
                                    NCSearchFriendsOperationQueueSpecificKey, NULL);
    }
    return self;
}
...
- (void)viewController:(UIViewController *)viewController
    ...
    if ([self.delegate respondsToSelector:@selector(searchFriendsViewModel:viewController:tableView:
...
- (UISearchBar *)configureSearchBarForViewController:(UIViewController *)viewController {
    if ([self.delegate
            respondsToSelector:@selector(
                                   willConfigureSearchBarViewModelForSearchFriendsViewModel:)]) {
        self.searchBarVM =
            [self.delegate willConfigureSearchBarViewModelForSearchFriendsViewModel:self];
...
- (NSArray *)configureRightNaviItemsForViewController:(UIViewController *)viewController {
    if ([self.delegate
            respondsToSelector:@selector(
                                   willConfigureRightNavigationItemsForSearchFriendsViewModel:)]) {
        self.naviItemsVM =
            [self.delegate willConfigureRightNavigationItemsForSearchFriendsViewModel:self];
    }
...
- (void)configureDataSourceWithArray:(NSArray<NCFriendInfo *> *)friendInfos {
    [self configureDataSourceWithArray:friendInfos
                             ...
                             requestId:self.searchRequestId];
}
...
- (void)configureDataSourceWithArray:(NSArray<NCFriendInfo *> *)friendInfos
    ...
    [self performOperationQueueBlock:^{
      ...
      // 通知用户修改数据源
      if ([self.delegate
              respondsToSelector:@selector(searchFriendsViewModel:willLoadItemsInDataSource:)]) {
          array = [self.delegate searchFriendsViewModel:self willLoadItemsInDataSource:tmp];
      }
      if (array) {
