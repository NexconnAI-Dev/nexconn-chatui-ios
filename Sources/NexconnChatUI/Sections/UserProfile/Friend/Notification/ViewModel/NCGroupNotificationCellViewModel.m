...
+ (void)registerCellForTableView:(UITableView *)tableView {
    [tableView registerClass:[NCGroupNotificationCell class] forCellReuseIdentifier:NCGroupNotificationCellIdentifier];
}
...
- (void)fetchGroupNameIfNeed {
    ...
    if (self.application.groupId) {
        [NCGroupChannel getGroupsInfoWithGroupIds:@[self.application.groupId]
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    NCGroupNotificationCell *cell = [tableView dequeueReusableCellWithIdentifier:NCGroupNotificationCellIdentifier
...
- (void)approveApplication {
    NCGroupChannel *channel = [[NCGroupChannel alloc] initWithChannelId:self.application.groupId ?: @""];
    if (!channel) {
        return;
    }
    // Received invitation.
    if (self.application.direction == NCGroupApplicationDirectionInvitationReceived) {
        [channel acceptInviteWithInviterId:self.application.inviterInfo.userId ?: @""
            ...
            if (error) {
                [self showTips:NCUILocalizedString(@"group_invitation_accept_failed")];
                return;
            }
        ...
        [channel acceptApplicationWithParams:params completion:^(NSInteger processCode, NCError * _Nullable error) {
            if (error) {
                [self showTips:NCUILocalizedString(@"group_other_invitation_accept_failed")];
                return;
            }
            if (processCode == NCChatUIErrorCodeGroupNeedInviteeAccept) { // An inviter is present.
                self.application.status = NCGroupApplicationStatusInviteeUnhandled;
            } else if(processCode == NCChatUIErrorCodeSuccess ) { // Join request.
                self.application.status = NCGroupApplicationStatusJoined;
            }
            [self reloadCell];
        }];
...
- (void)rejectApplication {
    ...
    if (self.application.direction == NCGroupApplicationDirectionInvitationReceived) {
        ...
        [channel refuseInviteWithParams:params completion:^(NCError * _Nullable error) {
            if (error) {
                [self showTips:NCUILocalizedString(@"group_invitation_refuse_failed")];
                return;
            }
        ...
        [channel refuseApplicationWithParams:params completion:^(NCError * _Nullable error) {
            if (error) {
                [self showTips:NCUILocalizedString(@"group_other_invitation_refuse_failed")];
                return;
            }
         
            if (self.application.status == NCGroupApplicationStatusAdminUnhandled) {
                self.application.status = NCGroupApplicationStatusAdminRefused;
            } else if (self.application.status == NCGroupApplicationStatusInviteeRefused) {
                self.application.status = NCGroupApplicationStatusInviteeRefused;
            }
            [self reloadCell];
        }];
...
- (NSString *)tipsOfOperator:(NCGroupApplicationInfo *)application {
    ...
    switch (application.direction) {
            // Received invitation.
        case NCGroupApplicationDirectionInvitationReceived:
            tips = [NSString stringWithFormat:NCUILocalizedString(@"group_notification_invite_me"), name];
            break;
            // Received application.
        case NCGroupApplicationDirectionApplicationReceived:
            if (application.inviterInfo) {
                tips = [NSString stringWithFormat:NCUILocalizedString(@"group_notification_invite_user_join_group"),name, application.joinMemberInfo.name];
            } else {
                tips = [NSString stringWithFormat:NCUILocalizedString(@"group_notification_apply_join_group"), name];
            }
            break;
        case NCGroupApplicationDirectionApplicationSent:
            ...
            break;
        case NCGroupApplicationDirectionInvitationSent:
            tips = [NSString stringWithFormat:NCUILocalizedString(@"group_notification_invite_other"), name];
            break;
        default:
            break;
    }
