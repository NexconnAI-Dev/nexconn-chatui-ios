...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&onceToken, ^{
      if (!defaultManager) {
          ...
          defaultManager.requestQueue =
              dispatch_queue_create("ai.nexconn.userInfoRequsetQueue", NULL);
...
- (void)setCurrentUserId:(NSString *)currentUserId {
    if ([NCChatUI shared].enablePersistentUserInfoCache &&
        ![currentUserId isEqualToString:_currentUserId]) {
        dispatch_async(self.dbQueue, ^{
          NSString *libraryPath = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory,
          ...
          NSString *libraryStoragePath =
              [libraryPath stringByAppendingPathComponent:@"ChatUIUserInfoCache"];
    ...
    if (userId) {
        ...
        if (!cacheUserInfo) {
            if (![NCChatUI shared].userInfoDataSource) {
                NCLogReleaseW(
                    @"User info provider is nil. Set [NCChatUI shared].userInfoDataSource "
                    @"and retain the assigned object because [NCChatUI shared].userInfoDataSource "
                    @"is a weak "
                    @"property...................");
                return nil;
            }
            if ([NCChatUI shared].userInfoDataSource &&
                [[NCChatUI shared].userInfoDataSource
                      ...
                      getUserInfoWithUserId:userId
                                 completion:^(NCChatUIUserInfo *userInfo) {
                                   if (userInfo == nil || userInfo.name.length == 0) {
                                       NCLogE(
                                           @"getUserInfo:error;;;getUserInfoDataSource:userId=%@",
                                           userId);
                                   }
...
- (NCChatUIUserInfo *)getUserInfo:(NSString *)userId inGroupId:(NSString *)groupId {
    ...
    if (userId && groupId) {
        NCChatUIUserInfo *cacheUserInfo =
            [[NCConversationUserInfoCache sharedCache] getUserInfo:userId
                                                       channelType:NCChannelTypeGroup
                                                         channelId:groupId];
        if (!cacheUserInfo && [NCChatUI shared].groupUserInfoDataSource &&
            [[NCChatUI shared].groupUserInfoDataSource
                             ...
                             completion:^(NCChatUIUserInfo *userInfo) {
                               if (!userInfo) {
                                   ...
                                   NCLogE(@"getUserInfo:inGroupId: "
                                          @"error;;;groupUserInfoDataSource;;;groupId=%@;;;"
                                          @"userInfo:userId=%@",
                                          groupId, userId);
                               }
        ...
        if (userInfo == nil || userInfo.name.length == 0) {
            NCLogE(@"getUserInfo:inGroupId: "
                   @"error;;;groupId=%@;;;cacheUserInfo:userId=%@,userName=%@,userPortraitUri=%@",
...
- (NCChatUIUserInfo *)fallBackOrdinaryUserInfo:(NCChatUIUserInfo *)tempUserInfo
...
- (NCChatUIUserInfo *)fallBackOrdinaryUserInfoFromCacheOnly:(NCChatUIUserInfo *)tempUserInfo
                                                  forUserId:(NSString *)userId {
    if (!tempUserInfo) {
        return [self getUserInfoFromCacheOnly:userId];
    }
...
- (void)clearGroupUserInfoNetworkCacheOnly:(NSString *)userId inGroup:(NSString *)groupId {
    if (userId && groupId) {
        [[NCConversationUserInfoCache sharedCache]
...
- (NCChatUIGroup *)getGroupInfo:(NSString *)groupId {
    if (groupId) {
        NCChannelInfo *cacheConversationInfo =
            [[NCChannelInfoCache sharedCache] getConversationInfo:NCChannelTypeGroup
                                                        channelId:groupId];
        if (!cacheConversationInfo && [NCChatUI shared].groupInfoDataSource &&
            [[NCChatUI shared].groupInfoDataSource
                respondsToSelector:@selector(getGroupInfoWithGroupId:completion:)]) {
            dispatch_async(self.requestQueue, ^{
              [[NCChatUI shared].groupInfoDataSource
                  getGroupInfoWithGroupId:groupId
                               completion:^(NCChatUIGroup *groupInfo) {
                                 NCLogI(@"getUserInfo:;;;getGroupInfoDataSource:groupId=%@,"
                                        @"groupName=%@,"
                                        @"groupPortraitUri=%@,extra=%@",
                                        groupInfo.groupId, groupInfo.groupName, groupInfo.avatarUrl,
                                        groupInfo.extra);
        ...
        NCLogI(
            @"getGroupInfo:;;;cacheGroupInfo:groupId=%@,groupName=%@,groupPortraitUri=%@,extra=%@",
...
- (void)updateGroupInfo:(NCChatUIGroup *)groupInfo forGroupId:(NSString *)groupId {
    if (groupId.length > 0 && !groupInfo) {
        [[NCChannelInfoCache sharedCache] clearConversationInfo:NCChannelTypeGroup
                                                      channelId:groupId];
        return;
    }
