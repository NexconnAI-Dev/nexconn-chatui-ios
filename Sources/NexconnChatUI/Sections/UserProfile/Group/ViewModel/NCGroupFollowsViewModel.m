...
- (void)fetchGroupFollows {
    ...
    [channel getFavoritesWithCompletion:^(NSArray<NCGroupFavoriteInfo *> *_Nullable followInfos,
...
- (void)actionButtonDidClick:(NCGroupFollowCellViewModel *)cellViewModel {
    ...
    NSString *message =
        [NSString stringWithFormat:NCUILocalizedString(@"remove_group_follows_alert"), name];
    [NCAlertView
        showAlertController:nil
             ...
             preferredStyle:(UIAlertControllerStyleAlert)actionsBlock:nil
               ...
               confirmBlock:^{
                 ...
                 [channel
                     removeFavoritesWithUserIds:@[ cellViewModel.memberInfo.userId ?: @"" ]
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    return [self.mutableFollowList[indexPath.row] tableView:tableView
...
- (void)getDetailInfo:(void (^)(NSArray<NCGroupFollowCellViewModel *> *cellVMs))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      ...
      for (int i = 0; i < self.followUserIds.count; i += 100) {
          NSArray *tempUserIds = [self.followUserIds
              subarrayWithRange:NSMakeRange(i, MIN(100, self.followUserIds.count - i))];
...
- (NSArray<NCGroupFollowCellViewModel *> *)getFollowsCellViewModels:(NSArray *)userIds {
    dispatch_semaphore_t semaphore =
        dispatch_semaphore_create(0); // Coordinate the two asynchronous requests.
                                            ...
                                            complete:^(
                                              ...
                                              [list addObjectsFromArray:
                                                        [self processMembers:members
                                                             withFriendInfos:friendInfos]];
                                              dispatch_semaphore_signal(
                                                  semaphore); // Mark the friend request complete.
                                            }];
                   }];
...
- (NSArray<NCGroupFollowCellViewModel *> *)processMembers:(NSArray<NCGroupMemberInfo *> *)members
                                          withFriendInfos:(NSArray<NCFriendInfo *> *)friendInfos {
...
- (void)resetMutableFollowList {
    ...
    NCGroupMemberAdditionalCellViewModel *addVM = [[NCGroupMemberAdditionalCellViewModel alloc]
        initWithTitle:NCUILocalizedString(@"add_follows_member")
             portrait:image];
...
- (void)pushSelectVC:(UIViewController *)viewController {
    NCSelectGroupMemberViewModel *viewModel =
        [NCSelectGroupMemberViewModel viewModelWithGroupId:self.groupId
                                           existingUserIds:self.followUserIds];
    ...
    [viewModel setSelectionDidCompelteBlock:^(NSArray<NSString *> *_Nonnull selectUserIds,
      ...
      [channel addFavoritesWithUserIds:selectUserIds
                            completion:^(NCError *_Nullable error) {
                              if (error) {
                                  dispatch_async(dispatch_get_main_queue(), ^{
                                    [NCAlertView
