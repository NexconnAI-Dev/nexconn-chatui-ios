...
+ (instancetype)viewModelWithGroupId:(NSString *)groupId {
    ...
    viewModel.groupEventHandlerId =
        [NSString stringWithFormat:@"nc.group.management.%p", viewModel];
    ...
    [NCGroupChannel getGroupsInfoWithGroupIds:@[ self.groupId ]
                                   completion:^(NSArray<NCGroupInfo *> *_Nullable groupInfos,
                                     ...
                                     if (groupInfos.firstObject &&
                                         (groupInfos.firstObject.role == NCGroupMemberRoleOwner ||
                                          groupInfos.firstObject.role == NCGroupMemberRoleAdmin)) {
...
- (void)viewController:(UIViewController *)viewController
    ...
    if ([self.delegate respondsToSelector:@selector(groupManagement:viewController:tableView:
                                                    didSelectRow:cellViewModel:)]) {
        BOOL intercept = [self.delegate groupManagement:self
    ...
    if ([commonCellVM.title isEqualToString:NCUILocalizedString(@"group_manager_title")]) {
        NCGroupManagerListViewModel *viewModel =
            [NCGroupManagerListViewModel viewModelWithGroupId:self.groupId];
        NCGroupManagerListController *vc =
            [[NCGroupManagerListController alloc] initWithViewModel:viewModel];
        [viewController.navigationController pushViewController:vc animated:YES];
    } else if ([commonCellVM.title
                   isEqualToString:NCUILocalizedString(@"set_group_info_edit_per")]) {
        [self setGroupInfoPermission];
    } else if ([commonCellVM.title
                   isEqualToString:NCUILocalizedString(@"set_add_group_member_per")]) {
        [self setAddGroupMemberPermission];
    } else if ([commonCellVM.title
                   isEqualToString:NCUILocalizedString(@"set_remove_group_member_per")]) {
        [self setRemoveGroupMemberPermission];
    } else if ([commonCellVM.title
                   isEqualToString:NCUILocalizedString(@"set_group_member_info_edit_per")]) {
        [self setGroupMemberInfoPermission];
    } else if ([commonCellVM.title isEqualToString:NCUILocalizedString(@"group_transfer")]) {
        NCGroupTransferViewModel *viewModel =
            [NCGroupTransferViewModel viewModelWithGroupId:self.groupId];
        NCGroupTransferViewController *vc =
            [[NCGroupTransferViewController alloc] initWithViewModel:viewModel];
        [viewController.navigationController pushViewController:vc animated:YES];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    return [self.dataSources[indexPath.section][indexPath.row] tableView:tableView
...
- (void)reloadDataSources:(NCGroupInfo *)group {
    ...
    NCProfileCommonCellViewModel *adminVM = [[NCProfileCommonCellViewModel alloc]
        initWithCellType:NCUProfileCellTypeText
                   title:NCUILocalizedString(@"group_manager_title")
                  detail:nil];
    [list addObject:@[ adminVM ]];
    if (group.role == NCGroupMemberRoleOwner) {
        NCProfileCommonCellViewModel *groupInfoEditPerVM = [[NCProfileCommonCellViewModel alloc]
            initWithCellType:NCUProfileCellTypeText
                       title:NCUILocalizedString(@"set_group_info_edit_per")
                      detail:[self groupOperationPerStr:group.groupInfoEditPermission]];
        NCProfileCommonCellViewModel *addGroupMemberPerVM = [[NCProfileCommonCellViewModel alloc]
            initWithCellType:NCUProfileCellTypeText
                       title:NCUILocalizedString(@"set_add_group_member_per")
                      detail:[self groupOperationPerStr:group.invitePermission]];
        NCProfileCommonCellViewModel *removeGroupMemberPerVM = [[NCProfileCommonCellViewModel alloc]
            initWithCellType:NCUProfileCellTypeText
                       title:NCUILocalizedString(@"set_remove_group_member_per")
                      detail:[self groupOperationPerStr:group.removeMemberPermission]];
        NCProfileCommonCellViewModel *memberInfoEditPerVM = [[NCProfileCommonCellViewModel alloc]
            initWithCellType:NCUProfileCellTypeText
                       title:NCUILocalizedString(@"set_group_member_info_edit_per")
                      detail:[self getMemberInfoPerStr:group.memberInfoEditPermission]];
        [list addObject:@[
            groupInfoEditPerVM, addGroupMemberPerVM, removeGroupMemberPerVM, memberInfoEditPerVM
        ]];
        ...
        NCProfileCommonCellViewModel *groupTransferVM = [[NCProfileCommonCellViewModel alloc]
            initWithCellType:NCUProfileCellTypeText
                       title:NCUILocalizedString(@"group_transfer")
                      detail:nil];
...
- (void)updateGroupJoinPermission:(BOOL)open {
    ...
    params.joinPermissionValue =
        @(open ? NCGroupJoinPermissionOwnerOrAdminVerify : NCGroupJoinPermissionFree);
...
- (void)setGroupInfoPermission {
    NSArray *permissonStrs = @[
        ...
        NCUILocalizedString(@"group_owner_or_manager_operation"),
        NCUILocalizedString(@"all_group_member_operation")
    ];
    [NCActionSheetView showActionSheetView:nil
                                 cellArray:permissonStrs
                             ...
                             selectedBlock:^(NSInteger index) {
                               ...
                               NCUpdateGroupInfoParams *params = [NCUpdateGroupInfoParams new];
                               params.groupInfoEditPermissionValue =
                                   @((NCGroupOperationPermission)index);
...
- (void)setRemoveGroupMemberPermission {
    ...
    [NCActionSheetView showActionSheetView:nil
                             ...
                             selectedBlock:^(NSInteger index) {
                               ...
                               NCUpdateGroupInfoParams *params = [NCUpdateGroupInfoParams new];
                               params.removeMemberPermissionValue =
                                   @((NCGroupOperationPermission)index);
                               [self updateGroupInfo:params reloadWithFailed:NO];
                             }
                               cancelBlock:^{
...
- (void)setGroupMemberInfoPermission {
    ...
    [NCActionSheetView showActionSheetView:nil
                             ...
                             selectedBlock:^(NSInteger index) {
                               ...
                               NCUpdateGroupInfoParams *params = [NCUpdateGroupInfoParams new];
                               params.memberInfoEditPermissionValue =
                                   @((NCGroupMemberInfoEditPermission)index);
                               [self updateGroupInfo:params reloadWithFailed:NO];
                             }
                               cancelBlock:^{
