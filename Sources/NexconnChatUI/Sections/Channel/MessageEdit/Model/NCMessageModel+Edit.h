...
/// Returns the message's own clientId if it is a reference message whose referenced content needs
/// refreshing; otherwise returns nil.
...
/// Returns the text content of the current editable message. Only valid for text and reference
/// messages.
...
/// Returns the formatted display text for the referenced message, or nil when there is no
/// referenced message.
- (nullable NSString *)edit_formattedReferencedMessageContent;

/// Updates the referenced message preview text from a new message model using the existing
/// same-type rules. Has no effect when it does not match.
- (void)edit_updateReferencedMessagePreviewContentFromModel:(nullable NCMessageModel *)model;

/// Updates the referenced message content from a new message model. Has no effect for non-reference
/// messages.
