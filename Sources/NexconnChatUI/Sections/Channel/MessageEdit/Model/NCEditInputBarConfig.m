            ...
            if (stateData) {
                ...
                if (mentionedRangeInfoList) {
                    ...
                    for (NSString *encodedInfo in mentionedRangeInfoList) {
                        NCMentionedStringRangeInfo *info = [[NCMentionedStringRangeInfo alloc] initWithDecodeString:encodedInfo];
                        if (info) {
                ...
                NSDictionary *referenceInfo = [stateData nc_dictionaryForKey:stateDataReferenceInfoKey];
                if (referenceInfo) {
                    self.referencedSenderName = [referenceInfo nc_stringForKey:stateDataReferenceSenderNameKey];
                    self.referencedContent = [referenceInfo nc_stringForKey:stateDataTextContentKey];
                    self.referencedMsgStatus = (NCReferenceMessageStatus)[referenceInfo nc_intForKey:stateDataReferenceStatusKey];
                }
            }
        }
    }
...
- (NSString *)encode {
    ...
    if (_referencedSenderName || _referencedContent) {  
        ...
        [referenceInfo nc_setObject:self.referencedSenderName forKey:stateDataReferenceSenderNameKey];
        [referenceInfo nc_setObject:self.referencedContent forKey:stateDataTextContentKey];
        [referenceInfo nc_setObject:@((NSInteger)self.referencedMsgStatus) forKey:stateDataReferenceStatusKey];
