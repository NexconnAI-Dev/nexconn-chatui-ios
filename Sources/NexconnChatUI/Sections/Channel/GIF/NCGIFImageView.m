...
@optional
- (void)debug_animatedImageView:(NCGIFImageView *)animatedImageView
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<NCGIFImageViewDebugDelegate> debug_delegate;
#endif

@end

@implementation NCGIFImageView
...
- (void)setAnimatedImage:(NCGIFImage *)animatedImage {
...
- (void)updateShouldAnimate {
    ...
    if (@available(iOS 14.0, *)) {
        if (!window) {
            window = [NCChatUIUtility getKeyWindow];
        }
    }
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        NCLog(NCGIFImageLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            NCLog(NCGIFImageLogLevelVerbose, @"Showing frame %lu for animated image: %@",
