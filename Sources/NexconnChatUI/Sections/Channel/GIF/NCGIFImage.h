...
//
//  An `NCGIFImage`'s job is to deliver frames in a highly performant way and works in conjunction with
//  `NCGIFImageView`.
...
// On success, the initializers return an `NCGIFImage` with all fields initialized, on failure they return `nil` and an
...
#define NCLog(logLevel, format, ...)                                                                                   \
    [NCGIFImage logStringFromBlock:^NSString * {                                                                       \
