...
- (NSDictionary *)attributeDictionaryForTextType:(NSTextCheckingTypes)textType {
    ...
    if (!linkColor) {
        linkColor = [NCChatUIUtility generateDynamicColor:[UIColor blueColor]
                                                darkColor:HEXCOLOR(0xFFBE6a)];
    }
    switch (textType) {
...
- (void)handleTap:(UITapGestureRecognizer *)gestureRecognizer {
    ...
    case NSTextCheckingTypePhoneNumber:
        if ([self.delegate
                respondsToSelector:@selector(attributedLabel:didSelectLinkWithPhoneNumber:)]) {
...
- (void)generateAttributedStrings {
    ...
    NSError *error = nil;
    NSDataDetector *dataDetector = [[NSDataDetector alloc] initWithTypes:self.textCheckingTypes
                                                                   error:&error];
    if (error != nil) {
    ...
    // Collect all matches before generating attributes once, including when there are no matches,
    // so suffix styles are still applied.
...
- (void)generateAttributedString {
    ...
    for (NSTextCheckingResult *textCheckingResult in self.attributedStrings) {
        for (int i = 0; i < textCheckingResult.numberOfRanges; i++) {
            ...
            NSDictionary *attributeDictionary =
                [self attributeDictionaryForTextType:textCheckingResult.resultType];

            if (NSEqualRanges(range, self.rangeOfTextHighlighted))
                attributeDictionary =
                    [self highlightedAttributeDictionaryForTextType:textCheckingResult.resultType];
            if (attributeDictionary) {
                if (self.originalString.length >= (range.location + range.length)) {
                    NSAttributedString *subString = [[NSAttributedString alloc]
                        initWithString:[self.originalString substringWithRange:range]
    ...
    // Apply the edited-suffix style only to the externally supplied range so matching user-entered
    // text is not misclassified.
    @try {
        if (self.nc_editedSuffixRange.location != NSNotFound &&
            ...
            if ([actual isEqualToString:editedSuffix]) {
                ...
                [attributedString addAttribute:NSForegroundColorAttributeName
                                         value:editedColor
                                         range:self.nc_editedSuffixRange];
            }
        }
...
- (NSUInteger)characterIndexAtPoint:(CGPoint)p {
    ...
    /**
     Core Text's suggested frame height does not include the empty final line represented by a
     single trailing newline. Append another newline so the suggested height and generated frame
     include that final line during hit testing.
     */
    if (optimizedAttributedText.string.length > 0 &&
        [[optimizedAttributedText.string
            substringFromIndex:optimizedAttributedText.string.length - 1] isEqualToString:@"\n"]) {
        [optimizedAttributedText
            appendAttributedString:[[NSAttributedString alloc] initWithString:@"\n"]];
    }
    // use label's font and lineBreakMode properties in case the attributedText does not contain
    // such attributes
    [optimizedAttributedText
    ...
    CGSize textSize = CTFramesetterSuggestFrameSizeWithConstraints(
        framesetter, CFRangeMake(0, [optimizedAttributedText length]), NULL,
        CGSizeMake(self.bounds.size.width, CGFLOAT_MAX), NULL);
