...
+ (void)showText:(NSString *)text
        ...
        delegate:(nonnull id<NCTextPreviewViewDelegate>)delegate {
...
- (instancetype)initWithFrame:(CGRect)frame text:(NSString *)text clientId:(long)clientId {
    if (self = [super initWithFrame:frame]) {
        ...
        CGFloat textHeight =
            [NCChatUIUtility getTextDrawingSize:text
                                           font:[UIFont systemFontOfSize:20]
                                constrainedSize:CGSizeMake(SCREEN_WIDTH - 20, MAXFLOAT)]
                .height;
        ...
        UITapGestureRecognizer *tap =
            [[UITapGestureRecognizer alloc] initWithTarget:self
                                                    action:@selector(didTapCurrentView)];
...
- (void)attributedLabel:(NCAttributedLabel *)label
    ...
    if ([self.textPreviewDelegate
            respondsToSelector:@selector(didTapPhoneNumberInMessageCell:model:)]) {
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.state ==
        UIGestureRecognizerStateBegan) { // Handle the gesture only when it begins.
        ...
        if (label && [label isKindOfClass:[UILabel class]]) {
            ...
            [[UIMenuController sharedMenuController]
                setMenuItems:[NSArray arrayWithObject:copyItem]];
...
- (UILabel *)label {
    if (!_label) {
        CGFloat textY = self.contentSize.height < SCREEN_HEIGHT
                            ? (SCREEN_HEIGHT - self.contentSize.height) / 2
                            : 0;
        _label = [[NCAttributedLabel alloc]
            initWithFrame:CGRectMake(10, textY, SCREEN_WIDTH - 20, self.contentSize.height)];
        _label.numberOfLines = 0;
        _label.textColor = NCDynamicColor(@"text_primary_color");
        _label.font = [UIFont systemFontOfSize:20];
        _label.textAlignment = NSTextAlignmentCenter;
        _label.attributeDictionary =
            [NCMessageCellTool getTextLinkOrPhoneNumberAttributeDictionary:NCMessageDirectionReceive
                                                              linkColorKey:@"primary_color"];
        _label.highlightedAttributeDictionary =
            [NCMessageCellTool getTextLinkOrPhoneNumberAttributeDictionary:NCMessageDirectionReceive
                                                              linkColorKey:@"primary_color"];
