...
@implementation NCSightRecorder
    ...
    if (mediaType == kCMMediaType_Video) {
        ...
        if (self.firstSample) {
            if ([self.assetWriter startWriting]) {
                ...
                NCLogE(@"Failed to start writing.");
            }
            self.firstSample = NO;
        }
        ...
        if (self.assetWriterVideoInput.readyForMoreMediaData) {
            if (![self.assetWriterInputPixelBufferAdaptor appendPixelBuffer:imageBuffer
                ...
                NCLogE(@"Error appending pixel buffer.");
            }
        }
        ...
        if (self.assetWriterAudioInput.isReadyForMoreMediaData) {
            if (![self.assetWriterAudioInput appendSampleBuffer:sampleBuffer]) {
                NCLogE(@"Error appending audio sample buffer.");
            }
        }
    }
}
