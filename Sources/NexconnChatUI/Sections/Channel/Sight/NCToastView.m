...
- (void)changeSubViewsFrameAndAnimation:(NSString *)toastString {
    ...
    NSMutableDictionary *attrs = [NSMutableDictionary dictionaryWithObject:font
                                                                    forKey:NSFontAttributeName];
    CGSize size =
        [toastString
            boundingRectWithSize:CGSizeMake(screenWidth - kToastViewLeftSpace * 4, CGFLOAT_MAX)
    ...
    self.titleLabel.frame =
        CGRectMake(kToastViewLeftSpace, kMCToastViewTopSpace, size.width, size.height);
    self.bgView.frame =
        CGRectMake((screenWidth - _titleLabel.frame.size.width - kToastViewLeftSpace * 2) / 2,
                   (screenHeight - KToastViewBottomSpace - _titleLabel.frame.size.height -
                    kMCToastViewTopSpace * 2),
                   _titleLabel.frame.size.width + kToastViewLeftSpace * 2,
                   _titleLabel.frame.size.height + kMCToastViewTopSpace * 2);
