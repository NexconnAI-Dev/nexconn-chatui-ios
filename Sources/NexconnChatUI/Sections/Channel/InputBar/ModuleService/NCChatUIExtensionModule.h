...
/*!
 ...
 @return The handling result. YES means the module handles the alert and the SDK will not play a
 sound. NO means the module does not handle it and the SDK will use default behavior. When the app
 is in the foreground and inside the conversation of the new message, there is no sound or
 notification; otherwise a sound alert is played. When the app is in the background, a local
...
/*!
 ...
 @param message   The received message
 @param fromName  The source name. For private messages it is the sender's name; for group messages
 it is the group name.
 @param userInfo  LocalNotification userInfo. If the extension module posts a local notification, it
 must include this userInfo.

 @return The handling result. YES means the module handles the notification and the SDK will not
 show one. NO means the module does not handle it and the SDK will use default behavior. When the
 app is in the foreground and inside the conversation of the new message, there is no sound or
 notification; otherwise a sound alert is played. When the app is in the background, a local
 notification is shown.
 */
