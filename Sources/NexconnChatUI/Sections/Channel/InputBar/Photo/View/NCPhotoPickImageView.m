...
- (UIView *)typeBackgroundView {
    if (!_typeBackgroundView) {
        _typeBackgroundView = [[UIView alloc]
            initWithFrame:CGRectMake(0, self.bounds.size.height - 28, self.bounds.size.width, 28)];
        ...
        gradientLayer.colors = @[
            (__bridge id)NCMASKCOLOR(0xffffff, 0).CGColor,
...
- (NCBaseImageView *)videoIcon {
    if (!_videoIcon) {
        ...
        if ([NCChatUIUtility isRTL]) {
            frame = (CGRect){self.typeBackgroundView.frame.size.width - 6 - 19,
                             self.typeBackgroundView.frame.size.height - 21, 19, 19};
        }
