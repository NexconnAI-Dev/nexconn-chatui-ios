...
- (void)configPickerCellWithItem:(NCAssetModel *)model
                        delegate:(id<NCPhotoPickerCollectCellDelegate>)delegate {
...
- (void)onSelectButtonClick:(UIButton *)sender {
    ...
    if (self.assetModel.mediaType == PHAssetMediaTypeVideo &&
        NSClassFromString(@"NCSightCapturer")) {
        [[NCAssetHelper shareAssetHelper]
            getOriginVideoWithAsset:self.assetModel.asset
                             result:^(AVAsset *avAsset, NSDictionary *info,
                                      NSString *imageIdentifier) {
                               if (![[[NCAssetHelper shareAssetHelper]
                                       getAssetIdentifier:self.assetModel.asset]
...
- (void)changeMessageModelState:(BOOL)originState assetModel:(NCAssetModel *)assetModel {
    ...
    if (originState != currentState) {
        ...
        if (self.delegate &&
            [self.delegate respondsToSelector:@selector(didChangeSelectedState:model:)]) {
...
- (void)setupSubviews {
    ...
    [self.contentView addConstraints:[NSLayoutConstraint
                                         constraintsWithVisualFormat:@"V:|-2-[_selectbutton(44)]"
                                                             options:kNilOptions
                                                             metrics:nil
                                                               views:NSDictionaryOfVariableBindings(
                                                                         _selectbutton)]];
    [self.contentView addConstraints:[NSLayoutConstraint
                                         constraintsWithVisualFormat:@"H:[_selectbutton(44)]-2-|"
                                                             options:kNilOptions
                                                             metrics:nil
                                                               views:NSDictionaryOfVariableBindings(
                                                                         _selectbutton)]];
    ...
    if (!_photoImageView) {
        ...
        UITapGestureRecognizer *tap =
            [[UITapGestureRecognizer alloc] initWithTarget:self
                                                    action:@selector(didTapPhotoImageView)];
...
- (NCBaseButton *)selectbutton {
    if (!_selectbutton) {
        ...
        [_selectbutton setImage:NCDynamicImage(@"media_file_state_uncheck_img")
