...
@interface NCPhotosPickerController () <UICollectionViewDelegateFlowLayout,
                                        PHPhotoLibraryChangeObserver,
                                        NCPhotoPickerCollectCellDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[NCAssetHelper shareAssetHelper] getPhotosOfGroup:self.currentAsset
...
- (void)viewDidLoad {
    ...
    [self.collectionView registerClass:[NCPhotoPickerCollectCell class]
...
- (void)viewWillLayoutSubviews {
    ...
    _toolBar.frame =
        CGRectMake(0, SCREEN_HEIGHT - shouldBeSubtractionHeight - height, SCREEN_WIDTH, height);
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!self.disableFirstAppear) {
        dispatch_async(dispatch_get_main_queue(), ^{
          ...
          CGRect frame =
              CGRectMake(0, MAX(contentSize.height - size.height, 0), size.width, size.height);
...
- (void)didTapPickerCollectCell:(NCAssetModel *)selectModel {
    ...
    [self checkDownloadFailFromiCloud:selectModel
                                block:^(BOOL downloadFailFromiCloud) {
                                  [weakSelf doTapPhotoImageView:selectModel
                                                 isDownloadFail:downloadFailFromiCloud];
                                }];
}
...
- (void)enterPreviewCollectionViewController:(NCAssetModel *)selectModel {
    ...
    [previewController previewPhotosWithSelectArr:self.selectedAssets
                                 ...
                                 accordToIsSelect:NO];
    [previewController setFinishPreviewAndBackPhotosPicker:^(NSMutableArray *selectArr,
                                                             NSArray *assetPhotos, BOOL isFull) {
...
- (void)checkDownloadFailFromiCloud:(NCAssetModel *)model
                              block:(void (^)(BOOL downloadFailFromiCloud))block {
    ...
    void (^progressHandler)(double, NSError *, BOOL *, NSDictionary *) =
    ...
    if (model.mediaType == PHAssetMediaTypeImage) {
        [[NCAssetHelper shareAssetHelper]
            getOriginImageDataWithAsset:model
                                 result:^(NSData *photo, NSDictionary *info,
        ...
        [[NCAssetHelper shareAssetHelper]
            getOriginVideoWithAsset:model.asset
    ...
    if (!assetModel.isSelect) {
        if (self.selectedAssets.count >= 9) {
            [NCAlertView showAlertController:nil
                                     message:NCUILocalizedString(@"max_selected_photos")
        ...
        if (assetModel.mediaType == PHAssetMediaTypeVideo &&
            ...
            if (round(assetModel.duration) > durationLimit) {
                NSString *localizedString =
                    durationLimit < 60 ? NCUILocalizedString(@"selected_video_warning_fmt_second")
                                       : NCUILocalizedString(@"selected_video_warning_fmt");
...
- (void)createTopView {
    CGFloat height = 49 + [self getSafeAreaExtraBottomHeight];
    _toolBar =
        [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT - height, SCREEN_WIDTH, height)];
    ...
    [_previewBtn addTarget:self
                    action:@selector(previewBtnCliced:)
          forControlEvents:UIControlEventTouchUpInside];
    ...
    if ([NCChatUIUtility isRTL]) {
        [_toolBar addConstraints:[NSLayoutConstraint
                                     constraintsWithVisualFormat:@"V:|-13-[_previewBtn(30)]"
                                                         options:kNilOptions
                                                         metrics:nil
                                                           views:NSDictionaryOfVariableBindings(
                                                                     _previewBtn)]];
        [_toolBar addConstraints:[NSLayoutConstraint
                                     constraintsWithVisualFormat:@"V:|-13-[_btnSend(30)]"
                                                         options:kNilOptions
                                                         metrics:nil
                                                           views:NSDictionaryOfVariableBindings(
                                                                     _btnSend)]];
        [_toolBar addConstraints:[NSLayoutConstraint
                                     constraintsWithVisualFormat:@"H:[_btnSend]"
                                                         options:kNilOptions
                                                         metrics:nil
                                                           views:NSDictionaryOfVariableBindings(
                                                                     _btnSend)]];
                                     ...
                                     constraintsWithVisualFormat:@"V:|-13-[_previewBtn(30)]"
                                                         ...
                                                         metrics:nil
                                                           views:NSDictionaryOfVariableBindings(
                                                                     _previewBtn)]];
        [_toolBar addConstraints:[NSLayoutConstraint
                                     constraintsWithVisualFormat:@"H:[_previewBtn]"
                                                         options:kNilOptions
                                                         metrics:nil
                                                           views:NSDictionaryOfVariableBindings(
                                                                     _previewBtn)]];
    ...
    [_btnSend setTitleColor:NCDynamicResourceColor(@"control_title_white_color",
                                                   @"photoPicker_send_disable", @"0x9fcdfd")
    ...
    [_previewBtn setTitleColor:NCDynamicResourceColor(@"text_secondary_color",
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      PHFetchResultChangeDetails *albumChanges =
          [changeInstance changeDetailsForFetchResult:self.currentAsset];
      if (albumChanges) {
          ...
          for (int i = 0; i < albumChanges.insertedObjects.count; i++) {
              if ([albumChanges.insertedObjects[i] isKindOfClass:[PHAsset class]]) {
                  ...
                  if (!isContain) {
                      NCAssetModel *model =
                          [NCAssetModel modelWithAsset:albumChanges.insertedObjects[i]];
...
- (void)updateCachedAssets {
    ...
    CGRect visibleRect =
        (CGRect){self.collectionView.contentOffset.x, self.collectionView.contentOffset.y,
    ...
    NSArray<NSValue *> *addedRects = [self addedRectsBetween:self.previousPreheatRect
                                                         and:preheatRect];
    NSArray<NSValue *> *removedRects = [self removedRectsBetween:self.previousPreheatRect
                                                             and:preheatRect];
...
- (NSArray<PHAsset *> *)assetsInRects:(NSArray<NSValue *> *)rects {
    ...
    [rects
        enumerateObjectsUsingBlock:^(NSValue *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
          ...
          [attribtutes enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes *_Nonnull obj,
...
- (NSArray<NSValue *> *)addedRectsBetween:(CGRect)old and:(CGRect)new {
    if (CGRectIntersectsRect(old, new)) {
        ...
        if (CGRectGetMaxY(new) > CGRectGetMaxY(old)) {
            CGRect rect = (CGRect){new.origin.x, CGRectGetMaxY(old), new.size.width,
                                   CGRectGetMaxY(new) - CGRectGetMaxY(old)};
            [rects addObject:[NSValue valueWithCGRect:rect]];
        }
        if (CGRectGetMinY(old) > CGRectGetMinY(new)) {
            CGRect rect = (CGRect){new.origin.x, CGRectGetMinY(new), new.size.width,
...
- (NSArray<NSValue *> *)removedRectsBetween:(CGRect)old and:(CGRect)new {
    if (CGRectIntersectsRect(old, new)) {
        ...
        if (CGRectGetMaxY(new) < CGRectGetMaxY(old)) {
            CGRect rect = (CGRect){new.origin.x, CGRectGetMaxY(new), new.size.width,
                                   CGRectGetMaxY(old) - CGRectGetMaxY(new)};
            [rects addObject:[NSValue valueWithCGRect:rect]];
        }
        if (CGRectGetMinY(old) < CGRectGetMinY(new)) {
            CGRect rect = (CGRect){new.origin.x, CGRectGetMinY(old), new.size.width,
