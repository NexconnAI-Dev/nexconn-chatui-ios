...
- (void)viewDidLoad {
    ...
    self.collectionView.contentSize =
        CGSizeMake(SCREEN_WIDTH * self.previewPhotosArr.count, SCREEN_HEIGHT);
    self.collectionView.pagingEnabled = YES;
    UITapGestureRecognizer *gesture =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapAction:)];
    [self.collectionView addGestureRecognizer:gesture];

    [self.collectionView registerClass:[NCPhotoPreviewCollectCell class]
            forCellWithReuseIdentifier:reuseIdentifier];
    [self.collectionView registerClass:NCVideoPreviewCell.class
            forCellWithReuseIdentifier:videoCellReuseIdentifier];
    [self creatTopView];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // On iOS 14 and later, scrolling in viewWillAppear has no effect before the collection view
    // finishes layout. Delaying by 0.1 seconds avoids repeated scrolling from viewDidLayoutSubviews
    // during scrolling or rotation. Reference: https://www.jianshu.com/p/482703c25fb6
    if (@available(iOS 14.0, *)) {
        dispatch_after(
            dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)),
            dispatch_get_main_queue(), ^{
              [self.collectionView
                  scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.currentIndex
                                                              inSection:0]
        ...
        [self.collectionView
            scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self.currentIndex inSection:0]
                   atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally
                           animated:NO];
    }
...
+ (instancetype)imagePickerViewController {
    NCPhotoPreviewCollectionViewFlowLayout *flowLayout =
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    //         */
    //        NSArray *reversePhotoArray = [self.previewPhotosArr
    //        reverseObjectEnumerator].allObjects; model = reversePhotoArray[indexPath.row];
    //    }
    NCPhotoPreviewCollectCell *cell = nil;
    if (model.mediaType == PHAssetMediaTypeVideo && NSClassFromString(@"NCSightCapturer")) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:videoCellReuseIdentifier
...
- (void)tapAction:(UIGestureRecognizer *)gesture {
    ...
    NCAssetModel *model = self.previewPhotosArr[indexPath.row];
    if (model.mediaType == PHAssetMediaTypeVideo && NSClassFromString(@"NCSightCapturer")) {
        NCVideoPreviewCell *videoCell =
            (NCVideoPreviewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
        [videoCell stop];
    }
...
- (void)backButtonAction {
    self.finishPreviewAndBackPhotosPicker(self.selectedArr, self.allPhotosArr,
                                          self.fullButton.selected);
...
- (void)isSelectedButtonAction:(UIButton *)button {
    ...
    if (selectModel.isDownloadFailFromiCloud) {
        [NCAlertView showAlertController:nil
                                 message:NCUILocalizedString(@"download_fail_fromi_cloud")
    ...
    [self
        selectedImageCanSend:selectModel
                              ...
                              if (self.selectedArr.count < 9) {
                                  ...
                                  if (isNotContains) {
                                      if (model.mediaType == PHAssetMediaTypeVideo &&
                                          ...
                                          CGFloat durationLimit =
                                              NCChatUIConfigCenter.message.uploadVideoDurationLimit;
                                          if (round(model.duration) <= durationLimit) {
                                              ...
                                              NSString *localizedString =
                                                  durationLimit < 60
                                                      ? NCUILocalizedString(
                                                            @"selected_video_warning_fmt_second")
                                                      : NCUILocalizedString(
                                                            @"selected_video_warning_fmt");
                                              CGFloat limit = durationLimit < 60
                                                                  ? durationLimit
                                                                  : (durationLimit / 60.0f);
                                              NSString *format =
                                                  durationLimit < 60 ? @"%.0f" : @"%.1f";
                                              NSString *text =
                                                  [NSString stringWithFormat:format, limit];
                                              NSString *msg =
                                                  [NSString stringWithFormat:localizedString, text];
                                  ...
                                  [NCAlertView showAlertController:nil
                                                           message:NCUILocalizedString(
                                                                       @"max_selected_photos")
                                                       cancelTitle:NCUILocalizedString(@"i_know_it")
                                                  inViewController:self];
                              }
                          }
...
- (void)fullBtnCliced:(UIButton *)sender {
    ...
    if (_fullButton.selected) {
        ...
        _fullButton.titleLabel.text =
            [NSString stringWithFormat:@"%@", NCUILocalizedString(@"full_image")];
...
- (void)creatTopView {
    ...
    [stateButton setImage:NCDynamicImage(@"photo_preview_uncheck_img")
                 forState:UIControlStateNormal];
    [stateButton setImage:NCDynamicImage(@"photo_preview_check_img")
                 forState:UIControlStateSelected];
    [stateButton sizeToFit];
    stateButton.imageEdgeInsets = (UIEdgeInsets){12, 12, 12, 12};
    if (hasExtendedStatusBar) {
        stateButton.frame =
            CGRectMake(_topView.frame.size.width - 10 - 44, _topView.frame.size.height / 2, 44, 44);
    } else {
        stateButton.frame = CGRectMake(_topView.frame.size.width - 10 - 44,
                                       _topView.frame.size.height / 2 - 44 / 2, 44, 44);
    }
    // Apply RTL ordering.
    [NCSemanticContext swapFrameForRTL:backButton withView:stateButton];

    [stateButton addTarget:self
                    action:@selector(isSelectedButtonAction:)
          forControlEvents:UIControlEventTouchUpInside];
    [_topView addSubview:self.selectedButton = stateButton];
}

- (void)createBottomView {
    CGFloat safeAreaHomeBarHeight =
        [NCChatUIUtility getWindowSafeAreaInsetsForView:self.view].bottom;
    ...
    [_sendButton setTitleColor:NCDynamicResourceColor(@"control_title_white_color",
                                                      @"photoPreview_send_disable", @"0x959595")
                      forState:UIControlStateDisabled];
    [_sendButton setTitleColor:NCDynamicResourceColor(@"control_title_white_color",
                                                      @"photoPicker_send_normal", @"0x0099ff")
                      forState:UIControlStateNormal];
    [_sendButton setBackgroundColor:NCDynamicColor(@"disabled_color")];

    [_sendButton addTarget:self
                    action:@selector(sendImageMessageButton:)
          forControlEvents:UIControlEventTouchUpInside];
    ...
    [_editButton setTitleColor:NCDynamicResourceColor(@"text_secondary_color",
                                                      @"photoPreview_send_disable", @"0x959595")
                      forState:UIControlStateNormal];
    ...
    if ([NCChatUIUtility isRTL]) {
        [_bottomView addConstraints:[NSLayoutConstraint
                                        constraintsWithVisualFormat:@"V:|-13-[_fullButton(30)]"
                                                            options:kNilOptions
                                                            metrics:nil
        ...
        [_bottomView addConstraints:[NSLayoutConstraint
                                        constraintsWithVisualFormat:@"V:|-13-[_sendButton(30)]"
                                                            options:kNilOptions
                                                            metrics:nil
                                                              views:NSDictionaryOfVariableBindings(
                                                                        _sendButton)]];
        [_bottomView addConstraints:[NSLayoutConstraint
                                        constraintsWithVisualFormat:@"V:|-13-[_editButton(30)]"
                                                            options:kNilOptions
                                                            metrics:nil
                                                              views:NSDictionaryOfVariableBindings(
                                                                        _editButton)]];
    ...
    [_fullButton
        setTitleColor:NCDynamicResourceColor(@"control_title_white_color",
                                             @"photoPreview_original_normal_text", @"0x999999")
             forState:UIControlStateNormal];
    [_fullButton
        setTitleColor:NCDynamicResourceColor(@"control_title_white_color",
                                             @"photoPreview_original_selected_text", @"0xffffff")
             forState:UIControlStateSelected];
    [_fullButton setImage:NCDynamicImage(@"photo_preview_uncheck_img")
                 forState:UIControlStateNormal];
    [_fullButton setImage:NCDynamicImage(@"photo_preview_check_img")
                 forState:UIControlStateSelected];
...
- (void)selectedImageCanSend:(NCAssetModel *)selectModel
                    complete:(void (^)(BOOL canSend))completeBlock {
    if ([[selectModel.asset valueForKey:@"uniformTypeIdentifier"]
                                   ...
                                   dispatch_async(dispatch_get_main_queue(), ^{
                                     if (imageData.length >
                                         ...
                                         UIAlertController *alertController = [UIAlertController
                                             alertControllerWithTitle:NCUILocalizedString(
                                                                          @"gif_above_max_size")
                                                              message:nil
                                                       preferredStyle:UIAlertControllerStyleAlert];
                                         [alertController
                                             addAction:[UIAlertAction
                                                           actionWithTitle:NCUILocalizedString(
                                                                               @"ok")
...
- (void)_updateBottomSendImageCountButton {
    dispatch_async(dispatch_get_main_queue(), ^{
      if (self.selectedArr.count && self.bottomView) {
          ...
          if ([NCChatUIUtility isRTL]) {
              [self.sendButton
                  setTitle:[NSString stringWithFormat:@"(%lu) %@",
                                                      (unsigned long)self.selectedArr.count,
                                                      NCUILocalizedString(@"send")]
                  forState:(UIControlStateNormal)];
          } else {
              [self.sendButton
                  setTitle:[NSString stringWithFormat:@"%@ (%lu)", NCUILocalizedString(@"send"),
                                                      (unsigned long)self.selectedArr.count]
                  forState:(UIControlStateNormal)];
          }
      } else {
...
- (void)getImageSize:(CGFloat)size {
    ...
    self.fullButton.titleLabel.text =
        [NSString stringWithFormat:@"%@ (%@)", NCUILocalizedString(@"full_image"), imageSize];
    [self.fullButton
        setTitle:[NSString
                     stringWithFormat:@"%@ (%@)", NCUILocalizedString(@"full_image"), imageSize]
        forState:UIControlStateNormal];
}
    ...
    if (!_indicatorView) {
        ...
        if ([NCChatUIUtility isRTL]) {
            _indicatorView.frame =
                CGRectMake(CGRectGetMinX(self.fullButton.frame) - 20 - 6, 18, 20, 20);
        } else {
            _indicatorView.frame =
                CGRectMake(CGRectGetMaxX(self.fullButton.titleLabel.frame) + 20, 18, 20, 20);
        }
