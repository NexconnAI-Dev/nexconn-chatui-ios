...
@property (nonatomic, assign) BOOL pointingUp; // YES points toward the top edge; NO points toward the bottom edge.
@end
...
@property (nonatomic, strong) UIView *overlayView; // Transparent overlay that intercepts background taps
...
- (void)showMenuFromRect:(CGRect)targetRect
    ...
    [menuView configureWithMenuItems:menuItems actionHandler:^(NCMenuItem *menuItem, NSInteger index) {
    ...
    CGPoint arrowAnchorPoint = [self arrowAnchorPointForTargetRect:targetRectInWindow visibleBounds:visibleBounds];

    // Prefer the space above the target when the full container fits there; otherwise use the space below.
    ...
    self.containerView = [[NCMenuHolderView alloc] initWithFrame:CGRectMake(menuX, containerY, menuWidth, containerHeight)];
    ...
    arrowX = MAX(10, MIN(arrowX, menuWidth - arrowWidth - 10)); // Keep the pointer inside the menu edges.
    
    // Position the pointer on the edge nearest the target. The measured menu now uses explicit frames.
    ...
    UIViewAnimationOptions showAnimationOptions = UIViewAnimationOptionCurveEaseOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState;
    [UIView animateWithDuration:0.25 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:0 options:showAnimationOptions animations:^{
...
- (void)keyboardWillChangeFrame:(NSNotification *)notification {
    ...
    BOOL visible = !CGRectIsEmpty(keyboardFrame) && CGRectGetMinY(keyboardFrame) < CGRectGetMaxY(screenBounds);
...
- (BOOL)arrowShouldPointUpForTargetRect:(CGRect)targetRect
    ...
    CGPoint anchorPoint = [self arrowAnchorPointForTargetRect:targetRect visibleBounds:visibleBounds];
