...
- (void)onDeletedMessagesForAll:(NSArray<NCMessage *> *)messages {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (isCurrentMessageDeletedForAll) {
          if (self.presentedViewController) { // Dismiss the download cancellation alert before
                                              // handling deletion.
...
- (void)startFileDownLoad {
    ...
    // 使发送端撤回时底层 cancel_download_media_message(clientId) 能命中并取消在途下载，与 Android
    // 对齐。 引用文件（NCReferenceMessage）与合并转发预览（合成负 clientId）不满足条件，仍走
    // URL-based 下载。
    ...
    [NCBaseChannel downloadMediaUrl:mediaUrl
        ...
        completionHandler:^(NSString *_Nullable mediaPath, NCError *_Nullable error) {
          ...
          // URL-based 下载底层不写 URL→本地路径映射，此处补写，供重进详情页
          // fileLocalPathForRemoteURL: 检测已下载状态 （合并转发/引用文件的合成消息无持久化
          // clientId，只能靠该映射恢复）
...
- (void)cancelFileDownload {
    // 与 startFileDownLoad 对称：message-based 下载的文件消息用 message-based
    // 取消（rc_message[clientId]）。
    if (self.messageModel.clientId > 0 &&
        [self.messageModel.content isKindOfClass:[NCFileMessage class]]) {
    ...
    [NCBaseChannel
        cancelDownloadMediaUrl:mediaUrl
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      self.progressLabel.text = [NSString
          stringWithFormat:@"%@(%@/%@)", NCUILocalizedString(@"file_is_downloading"),
                           [NCChatUIUtility
                               getReadableStringForFileSize:progress * self.fileMessage.size],
...
- (BOOL)isFileDownloaded {
    if (self.fileMessage.localPath.length > 0 &&
        [NCFileUtility isFileExist:self.fileMessage.localPath]) {
        return YES;
    } else {
        return NO;
    }
...
- (BOOL)isFileSupported {
    if (![[NCChatUIUtility getFileTypeIcon:self.fileMessage.fileType]
            isEqualToString:@"OtherFile"]) {
        return YES;
    } else {
        return NO;
    }
...
- (NCBaseImageView *)typeIconView {
    if (!_typeIconView) {
        _typeIconView = [[NCBaseImageView alloc]
            initWithFrame:CGRectMake((self.view.bounds.size.width - 75) / 2, 64, 75, 75)];
...
- (UILabel *)nameLabel {
    if (!_nameLabel) {
        _nameLabel = [[UILabel alloc]
            initWithFrame:CGRectMake(10, CGRectGetMaxY(self.typeIconView.frame) + 16,
...
- (UILabel *)sizeLabel {
    if (!_sizeLabel) {
        _sizeLabel =
            [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.nameLabel.frame) + 12,
...
- (UILabel *)progressLabel {
    if (!_progressLabel) {
        _progressLabel =
            [[UILabel alloc] initWithFrame:CGRectMake(10, CGRectGetMaxY(self.nameLabel.frame) + 12,
                                                      self.view.bounds.size.width - 10 * 2, 16)];
...
- (NCBaseButton *)downloadButton {
    if (!_downloadButton) {
        _downloadButton = [[NCBaseButton alloc]
            initWithFrame:CGRectMake(10, CGRectGetMaxY(self.sizeLabel.frame) + 12,
...
- (NCBaseButton *)openInOtherAppButton {
    if (!_openInOtherAppButton) {
        _openInOtherAppButton = [[NCBaseButton alloc] initWithFrame:self.downloadButton.frame];
