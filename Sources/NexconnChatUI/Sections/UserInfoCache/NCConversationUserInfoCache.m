...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.conversationUserInfoCacheRWQueue =
            dispatch_queue_create("ai.nexconn.conversationUserInfoCacheRWQueue", NULL);
        dispatch_queue_set_specific(self.conversationUserInfoCacheRWQueue, cacheRWQueueTag,
                                    cacheRWQueueTag, NULL);
    }
    return self;
}
    ...
    if (![userInfo isEqual:cacheUserInfo]) {
        ...
        dispatch_async(ncUserInfoDBQueue, ^{
          [ncUserInfoWriteDBHelper replaceUserInfoFromDB:userInfo
                                             ...
                                             channelType:channelType
                                               channelId:channelId];
          NCLogI(@"updateUserInfo:forUserId:channelType;channelId:;;;channelType=%lu,targerId=%@,"
                 @"userId=%@,"
                 @"name=%@,portrait=%@",
                 (unsigned long)channelType, channelId, userInfo.userId, userInfo.name,
                 userInfo.avatarUrl);
...
- (void)clearConversationUserInfo:(NSString *)userId
    ...
    dispatch_async(ncUserInfoDBQueue, ^{
      [ncUserInfoWriteDBHelper deleteConversationUserInfoFromDB:userId
                                                    channelType:channelType
                                                      channelId:channelId];
      NCLogI(@"clearConversationUserInfo:channelType;channelId:;;;userId:%@,channelType=%lu,"
             @"targerId=%@",
             userId, (unsigned long)channelType, channelId);
...
- (void)clearAllConversationUserInfo {
    ...
    //    __weak typeof(self) weakSelf = self;
    dispatch_async(ncUserInfoDBQueue, ^{
      //        NSArray *dbUserInfoList = [ncUserInfoWriteDBHelper
      //        selectAllConversationUserInfoFromDB]; for (NCChatUIUserInfo *dbUserInfo in
      //        dbUserInfoList) {
      //            [weakSelf removeImageCache:dbUserInfo];
      //        }
