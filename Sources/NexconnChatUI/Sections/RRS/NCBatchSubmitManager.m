...
static NSString *const kNCBatchSubmitManagerConnectionStatusHandlerIdentifier =
    @"NCBatchSubmitManager";
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [NCEngine addConnectionStatusHandlerWithIdentifier:
                      kNCBatchSubmitManagerConnectionStatusHandlerIdentifier
                                                   handler:self];
    }
    return self;
}
...
- (void)addSubmitTask:(id)item {
    ...
    NCLogD(@"[%@] Added item to pending queue, total: %lu, state: %ld, connected: %d",
           kNCBatchSubmitManagerTag, (unsigned long)self.pendingItems.count,
           (long)self.currentState, self.isConnected);
    ...
    if (self.isConnected && self.currentState == NCSubmitStateIdle) {
        ...
        NCLogD(@"[%@] Not scheduling: connected=%d, state=%ld", kNCBatchSubmitManagerTag,
               self.isConnected, (long)self.currentState);
    }
...
- (void)scheduleDelayedSubmit {
    ...
    // The block runs later on the main queue, after the scheduling caller has normally released
    // _lock.
    NSTimeInterval delay =
        _delayMs / 1000.0; // Access the ivar directly while the caller holds _lock.
...
- (void)executeBatchSubmit {
    ...
    itemsToSubmit =
        [[self.pendingItems objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:range]] copy];
    ...
    NCLogD(@"[%@] Preparing to submit %lu items, remaining: %lu", kNCBatchSubmitManagerTag,
           (unsigned long)itemsToSubmit.count, (unsigned long)self.pendingItems.count);
    pthread_mutex_unlock(&_lock);
    ...
    if (itemsToSubmit.count > 0 && callback) {
        ...
        @try {
            callback(itemsToSubmit, ^(NSInteger code, BOOL refillData) {
              ...
              NCLogD(@"[%@] Batch submit result: %ld, refillData: %d", kNCBatchSubmitManagerTag,
                     (long)code, refillData);
        ...
        if (!callback) {
            NCLogD(@"[%@] No submit callback; discarding %lu items", kNCBatchSubmitManagerTag,
...
- (void)refillData:(NSArray *)itemsToRefill {
    ...
    NCLogD(@"[%@] Refilled %lu items to queue head, total pending: %lu", kNCBatchSubmitManagerTag,
           (unsigned long)itemsToRefill.count, (unsigned long)_pendingItems.count);
    pthread_mutex_unlock(&_lock);
}
...
- (void)onBatchSubmitComplete {
    ...
    NCLogD(@"[%@] onBatchSubmitComplete called, pending items: %lu, state: %ld, connected: %d",
           kNCBatchSubmitManagerTag, (unsigned long)_pendingItems.count, (long)_currentState,
           self.isConnected);
    ...
    if (_pendingItems.count == 0) {
        ...
        NCLogD(@"[%@] Has %lu pending items, scheduling next batch", kNCBatchSubmitManagerTag,
               (unsigned long)_pendingItems.count);
        [self scheduleDelayedSubmit];
    } else {
        // Return to idle while disconnected; connection recovery will restart submission.
        _currentState = NCSubmitStateIdle;
        NCLogD(@"[%@] Pending items remain while disconnected; waiting for connection recovery",
...
- (void)onConnectionStatusChanged:(NCConnectionStatusChangedEvent *)event {
    ...
    NCLogD(@"[%@] Connection status changed: %ld, pending items: %lu", kNCBatchSubmitManagerTag,
           (long)status, (unsigned long)self.pendingItems.count);
