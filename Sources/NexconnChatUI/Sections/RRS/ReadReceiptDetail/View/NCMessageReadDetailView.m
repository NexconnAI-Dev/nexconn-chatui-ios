...
- (UITableView *)createTableView {
    ...
    CGFloat leftOffset =
        [NCChatUIConfig defaultConfig].ui.globalConversationPortraitSize.width + 12;
    ...
    NCMJRefreshAutoNormalFooter *footer = [NCMJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
      ...
      if ([strongSelf.delegate
              respondsToSelector:@selector(readReceiptUserListView:needLoadMoreForTabType:)]) {
...
- (void)setupViewConstraints {
    ...
    [NSLayoutConstraint activateConstraints:@[
        [self.emptyContainerView.centerXAnchor
            constraintEqualToAnchor:self.tableContainerView.centerXAnchor],
        [self.emptyContainerView.centerYAnchor
            constraintEqualToAnchor:self.tableContainerView.centerYAnchor],
        [self.emptyContainerView.leadingAnchor
            constraintGreaterThanOrEqualToAnchor:self.tableContainerView.leadingAnchor
                                        constant:20],
        [self.emptyContainerView.trailingAnchor
            constraintLessThanOrEqualToAnchor:self.tableContainerView.trailingAnchor
                                     constant:-20]
    ]];
}
...
- (void)reloadDataForTabType:(NCMessageReadDetailTabType)tabType hasMoreData:(BOOL)hasMoreData {
    ...
    UITableView *tableView =
        tabType == NCMessageReadDetailTabTypeRead ? self.readTableView : self.unreadTableView;
