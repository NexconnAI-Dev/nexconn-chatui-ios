...
@interface NCMessageReadDetailViewController () <UITableViewDelegate, UITableViewDataSource,
                                                 NCMessageReadDetailViewModelResponder,
                                                 NCMessageReadDetailViewDelegate>
...
- (void)setupView {
    ...
    if (self.dataSource &&
        [self.dataSource respondsToSelector:@selector(viewController:headerViewWithMessage:)]) {
        self.headerView = [self.dataSource viewController:self
                                    headerViewWithMessage:self.viewModel.messageModel];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    NCMessageReadDetailCellViewModel *cellViewModel =
        [self.viewModel cellViewModelForTabType:tabType atIndex:indexPath.row];
    if (!cellViewModel) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                      reuseIdentifier:@"EmptyCell"];
    }

    NCMessageReadDetailCell *cell =
        [tableView dequeueReusableCellWithIdentifier:[NCMessageReadDetailCell reuseIdentifier]
...
- (NCMessageReadDetailView *)mainView {
    if (!_mainView) {
        ...
        // Register the cell.
        [_mainView.readTableView registerClass:[NCMessageReadDetailCell class]
                        forCellReuseIdentifier:[NCMessageReadDetailCell reuseIdentifier]];
        [_mainView.unreadTableView registerClass:[NCMessageReadDetailCell class]
                          forCellReuseIdentifier:[NCMessageReadDetailCell reuseIdentifier]];
    }
    return _mainView;
}
