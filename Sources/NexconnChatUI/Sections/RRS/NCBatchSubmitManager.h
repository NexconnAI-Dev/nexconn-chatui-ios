...
/**
 ...
 8. Connection-state awareness: task processing is automatically paused or resumed according to connection state.
...
/**
 Stops receiving connection status notifications while preserving internal state so remaining tasks can continue until completion.
 
 @note After this method is called, BatchSubmitManager no longer responds to connection state changes,
       but added tasks continue to execute until they complete or fail.
 */
- (void)invalidate;
