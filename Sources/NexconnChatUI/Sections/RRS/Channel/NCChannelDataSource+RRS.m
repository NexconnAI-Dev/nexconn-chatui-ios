...
// TODO: This still derives the NC channel and identifier from NCChannelViewController. Remove it
// when the page owns NC context directly.
...
- (void)rrs_fetchReadReceiptInfo:(NSArray<NCMessageModel *> *)models {
    ...
    if ([dic allKeys].count) {
        ...
        [channel
            getMessageReadReceiptInfoWithMessageIds:messageUIDs
                                         completion:^(NSArray<NCMessageReadReceiptInfo *>
                                                          *_Nullable infoList,
                                               ...
                                               for (NCMessageReadReceiptInfo *info in infoList) {
                                                   ...
                                                   dispatch_after(
                                                       ...
                                                       dispatch_get_main_queue(), ^{
                                                         [self.chatVC.util
                                                             sendMessageReadReceiptNotification:
                                                                 model];
                                                       });
                                               }
                                           }
...
- (void)rrs_respondReadReceipt:(NSDictionary *)dic {
    if ([dic allKeys].count) {
        ...
        [channel sendReadReceiptResponseWithMessageIds:messageUIDs
